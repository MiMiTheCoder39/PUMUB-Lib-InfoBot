...
from models.book_model import get_all_authors, get_all_books, get_all_categories, get_book_by_id
...
def _catalog_snapshot(max_books: int = 24) -> dict[str, Any]:
    """Live counts + a small sample of recent books from the library database."""
    try:
        books = get_all_books(limit=max_books)
        books = books or []
        categories = get_all_categories() or []
        authors = get_all_authors() or []
    except Exception:  # never let catalog lookup break the chat
    ...
    return {
        ...
        "recent_books": sample,
        "categories": [str(c.get("category_name")) for c in categories[:60]],
        "authors": [str(a.get("author_name")) for a in authors[:120]],
    }
...
def handle_chat(question: str, *, user_id: int, role: str | None, book_id: int | None = None, action: str | None = None, mode: str = "medium") -> dict[str, Any]:
    ...
    return {
        ...
        "answer": generate_response(
            question,
            system_prompt=(
                "You are LibInfoBot, a helpful digital-library assistant for "
                "Polytechnic University (Maubin). Answer briefly and accurately "
                "using the library catalog context below. When answering from "
                "the catalog context, state totals in numbers where relevant. "
                "If the question is about the catalog, mention how the user can "
                "search by title, author, category, or ISBN on the library."
                f"\n\nLibrary catalog context: {_catalog_snapshot()}"
            ),
        ),
        "books": [],
    }
